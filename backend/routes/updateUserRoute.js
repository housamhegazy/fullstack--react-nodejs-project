...
router.put('/update-profile', isAuthenticated, upload.single('avatar'), async (req, res) => {
  try {
    ...
    // Handle image upload
    if (req.file) {
        const publicId = req.user.avatar.split('/').pop().split('.')[0]; // Extract public ID from URL
